import { ANONYMOUS_MODE_BUTTON_SELECTOR, RESPONSES_CONTAINER_XPATH, STOP_BUTTON_SELECTOR, COPY_BUTTON_SELECTOR, GENERATION_TIMEOUT, ASK_INPUT_SELECTOR, NETWORK_IDLE_EVENT, PERPLEXITY_URL, CLIPBOARD_DELAY, BUTTON_WAIT, TYPING_DELAY, ENTER_KEY, DIV_TAG } from '../config/constants';

import type { Page } from 'puppeteer';

export const enableAnonymousMode = async (page: Page): Promise<boolean> => {
  try {
...
export const getAIResponseText = async (page: Page): Promise<string | null> => {
  try {
    ...
    return await page.evaluate(async (xpath: string, copySelector: string, delay: number, tag: string) => {
...
export const sendPromptToAI = async (page: Page, prompt: string): Promise<boolean> => {
  try {
...
export const resetPage = async (page: Page): Promise<boolean> => {
  try {
